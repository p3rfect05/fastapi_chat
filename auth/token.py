...
async def validate_token(token: str = Depends(get_token)) -> int | None:
    try:
        ...
        if user_id is None:
            raise HTTPException(
                detail="User unauthenticated", status_code=status.HTTP_401_UNAUTHORIZED
            )
...
async def get_current_user(
    response: Response, user_id: int = Depends(validate_token)
) -> User:
...
def create_access_token(data: dict) -> str:
    ...
    encoded_jwt = jwt.encode(to_encode, SECRET_KEY, HS_ALGORITHM)
    return encoded_jwt
